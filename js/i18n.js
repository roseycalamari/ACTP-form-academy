window.ACTP_I18N = {
  pt: {
    htmlLang: "pt",
    title: "ACTP · Disponibilidade de aulas · Pais",
    academy: "Academy",
    social: "Social",
    pineCliffs: "Pine Cliffs",
    brand: "Annabel Croft",
    brandSub: "Tennis & Padel",
    noCommitKicker: "Sem compromisso · apenas informação",
    noCommit:
      "Isto não é uma inscrição definitiva nem um pagamento. Basta preencher para nos dar informação — assim conseguimos formar turmas. Só confirmamos o lugar (e só se paga) quando a turma fechar, normalmente com 4 alunos.",
    lead:
      "Aulas locais de ténis e padel no Pine Cliffs. Diga-nos quando pode trazer o(a) seu/sua filho(a) — construímos as turmas a partir da vossa disponibilidade.",
    season: "Época 2026/27 · Novembro a Março/Abril · Pine Cliffs Resort, Albufeira.",
    whenTitle: "Aulas de segunda a sexta",
    whenSub: "Das 16h30 às 21h30 · a última sessão é das 20h30 às 21h30",
    studentInfo: "Dados do aluno",
    studentName: "Nome do aluno",
    age: "Idade",
    parentName: "Nome do encarregado de educação",
    phone: "Telemóvel / WhatsApp",
    email: "Email",
    school: "Escola (opcional)",
    sport: "Modalidade",
    tennis: "Ténis",
    padel: "Padel",
    both: "Ambos",
    timesTitle: "Quantas vezes por semana pode trazer?",
    timesHint:
      "Marque a opção mais realista. Pode indicar disponibilidade em mais dias do que as vezes por semana.",
    times1: "1× por semana",
    times2: "2× por semana",
    times3: "3× por semana",
    timesFlex: "Flexível",
    gridTitle: "Horários possíveis · marque todos os que possam servir",
    gridHint:
      "Escolha primeiro a cor da bola. Depois marque todas as combinações dia + hora possíveis — não só a preferida. Assim formamos turmas da mesma bola.",
    schedule: "Horário",
    mon: "Seg",
    tue: "Ter",
    wed: "Qua",
    thu: "Qui",
    fri: "Sex",
    monLong: "Segunda",
    tueLong: "Terça",
    wedLong: "Quarta",
    thuLong: "Quinta",
    friLong: "Sexta",
    selectedNone: "Nenhum horário selecionado ainda.",
    selectedCount: "{n} horário(s) possível(is)",
    howTitle: "Como funciona:",
    howBody:
      "Abrimos estes horários a todas as famílias. Quando uma turma chega ao mínimo (normalmente 4), fechamos esse dia e hora e confirmamos por WhatsApp. Só paga quando o lugar estiver confirmado.",
    prefTitle: "Preferência de horário (opcional)",
    prefHint: "Se tiver um 1.º e 2.º preferidos dentro das caixas que marcou:",
    firstChoice: "1.ª escolha · dia + hora",
    secondChoice: "2.ª escolha · dia + hora",
    chooseSlot: "Escolher…",
    tennisLevelTitle: "Cor da bola (ténis)",
    tennisLevelHint:
      "Red, Orange, Green e Yellow são os estágios da bola. Os números (ex.: ~5–8) são faixas etárias típicas, não o nível de jogo. Escolha a bola antes de marcar os horários.",
    ballRed: "Bola red",
    ballOrange: "Bola orange",
    ballGreen: "Bola green",
    ballYellow: "Bola yellow",
    ballUnknown: "Bola por confirmar",
    padelLevelTitle: "Padel (se marcou padel)",
    levelRed: "Red (idades ~5–8)",
    levelOrange: "Orange (idades ~8–9)",
    levelGreen: "Green (idades ~9–10)",
    levelYellow: "Yellow (idades 10+)",
    levelUnknown: "Não sei",
    padelBeginner: "Iniciante",
    padelIntermediate: "Intermédio",
    padelTeens: "Adolescentes",
    child2Title: "2.º filho (opcional)",
    child2Name: "Nome",
    child2Age: "Idade",
    child2Same: "Mesmos horários do 1.º",
    child2Different: "Horários diferentes (escreva nas notas)",
    notes: "Notas",
    notesPlaceholder: "Alguma informação extra que nos ajude a formar a turma…",
    confirmLabel:
      "Confirmo que estes dados estão corretos. Percebo que isto não é um compromisso — só serve para informação, e só pago se o lugar for confirmado.",
    submit: "Enviar disponibilidade",
    sending: "A enviar…",
    priceNote:
      "Preçário local (indicação): crianças €16 / hora · turmas a partir de 4 alunos. Pack e confirmação quando a turma fechar.",
    contactFooter: "Mais informação contacte",
    errRequired: "Por favor preencha os campos obrigatórios.",
    errSport: "Escolha ténis, padel ou ambos.",
    errTimes: "Indique quantas vezes por semana pode trazer.",
    errSlots: "Marque pelo menos um horário possível.",
    errConfirm: "Confirme que percebeu que isto não é um compromisso.",
    errPhone: "Indique um telemóvel / WhatsApp válido.",
    errEmail: "Indique um email válido, ou deixe o campo vazio.",
    successKicker: "Recebemos a ficha",
    successTitle: "Obrigado — sem compromisso.",
    successBody:
      "Já temos a vossa disponibilidade. Vamos formar turmas a partir destas respostas e contactamos por WhatsApp quando houver lugar. Não precisam de pagar nada agora.",
    successWhatsApp: "Enviar uma cópia por WhatsApp",
    successAgain: "Enviar outra ficha",
    successRef: "Referência",
    placeholderChoice: "Marque horários acima primeiro"
  },
  en: {
    htmlLang: "en",
    title: "ACTP · Class availability · Parents",
    academy: "Academy",
    social: "Social",
    pineCliffs: "Pine Cliffs",
    brand: "Annabel Croft",
    brandSub: "Tennis & Padel",
    noCommitKicker: "No commitment · information only",
    noCommit:
      "This is not a final enrolment and there is no payment yet. Just fill it in so we can gather information and form classes. We only confirm your spot (and you only pay) once a class is closed, usually with 4 students.",
    lead:
      "Tennis and padel lessons at Pine Cliffs. Let us know when you can bring your child — we organise the classes based on your availability.",
    season: "2026–27 season · November through March/April · Pine Cliffs Resort, Albufeira.",
    whenTitle: "Classes Monday through Friday",
    whenSub: "From 4:30 p.m. to 9:30 p.m. · the last session is from 8:30 p.m. to 9:30 p.m.",
    studentInfo: "Student information",
    studentName: "Name",
    age: "Age",
    parentName: "Name of the parent or guardian",
    phone: "Phone / WhatsApp",
    email: "Email",
    school: "School (optional)",
    sport: "Sport",
    tennis: "Tennis",
    padel: "Padel",
    both: "Both",
    timesTitle: "How many times a week can you bring your child?",
    timesHint:
      "Select the most realistic option. You can indicate availability on more days than the number of times per week.",
    times1: "1× per week",
    times2: "2× per week",
    times3: "3× per week",
    timesFlex: "Flexible",
    gridTitle: "Possible times · check all that apply",
    gridHint:
      "Pick the ball colour first. Then mark every possible day + time — not just the preferred one. That’s how we form classes of the same ball.",
    schedule: "Schedule",
    mon: "Mon",
    tue: "Tue",
    wed: "Wed",
    thu: "Thu",
    fri: "Fri",
    monLong: "Monday",
    tueLong: "Tuesday",
    wedLong: "Wednesday",
    thuLong: "Thursday",
    friLong: "Friday",
    selectedNone: "No time slots selected yet.",
    selectedCount: "{n} possible time slot(s)",
    howTitle: "How it works:",
    howBody:
      "We open these time slots to all families. When a class reaches the minimum number of participants (usually 4), we close that day and time slot and confirm it via WhatsApp. You only pay once your spot is confirmed.",
    prefTitle: "Preferred time (optional)",
    prefHint: "If you have a first and second choice among the boxes you checked:",
    firstChoice: "1st choice · day + time",
    secondChoice: "2nd choice · day + time",
    chooseSlot: "Choose…",
    tennisLevelTitle: "Ball colour (tennis)",
    tennisLevelHint:
      "Red, Orange, Green and Yellow are the ball stages. The numbers (e.g. ~5–8) are typical age groups, not a skill rating. Choose the ball before ticking times.",
    ballRed: "Red ball",
    ballOrange: "Orange ball",
    ballGreen: "Green ball",
    ballYellow: "Yellow ball",
    ballUnknown: "Ball to confirm",
    padelLevelTitle: "Padel (if “padel” was checked)",
    levelRed: "Red (ages ~5–8)",
    levelOrange: "Orange (ages ~8–9)",
    levelGreen: "Green (ages ~9–10)",
    levelYellow: "Yellow (ages 10+)",
    levelUnknown: "I don’t know",
    padelBeginner: "Beginner",
    padelIntermediate: "Intermediate",
    padelTeens: "Teenagers",
    child2Title: "2nd child (optional)",
    child2Name: "Name",
    child2Age: "Age",
    child2Same: "Same schedule as the 1st",
    child2Different: "Different times (please note them in the comments)",
    notes: "Notes",
    notesPlaceholder: "Anything else that helps us form the class…",
    confirmLabel:
      "I confirm this information is correct. I understand this is not a commitment — it is only for information, and I only pay if a spot is confirmed.",
    submit: "Send availability",
    sending: "Sending…",
    priceNote:
      "Local price list (for reference): children €16/hour · classes start with 4 students. Confirmation once the class reaches 4 students.",
    contactFooter: "For more information, please contact us",
    errRequired: "Please fill in the required fields.",
    errSport: "Please choose tennis, padel or both.",
    errTimes: "Please say how many times a week you can bring your child.",
    errSlots: "Please check at least one possible time slot.",
    errConfirm: "Please confirm you understand this is not a commitment.",
    errPhone: "Please enter a valid phone / WhatsApp number.",
    errEmail: "Please enter a valid email, or leave it blank.",
    successKicker: "We have your form",
    successTitle: "Thank you — no commitment.",
    successBody:
      "We have your availability. We will form classes from these answers and contact you on WhatsApp when there is a spot. You do not need to pay anything now.",
    successWhatsApp: "Send a copy via WhatsApp",
    successAgain: "Submit another form",
    successRef: "Reference",
    placeholderChoice: "Check time slots above first"
  }
};

window.ACTP_SLOTS = {
  days: ["mon", "tue", "wed", "thu", "fri"],
  times: [
    { id: "1630", label: "16h30 – 17h30" },
    { id: "1730", label: "17h30 – 18h30" },
    { id: "1830", label: "18h30 – 19h30" },
    { id: "1930", label: "19h30 – 20h30" },
    { id: "2030", label: "20h30 – 21h30" }
  ]
};
